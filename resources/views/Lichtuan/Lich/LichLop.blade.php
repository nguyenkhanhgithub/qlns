...
<div class="row">
	...
	<div class="col-md-12">
		<div class="white-box">
			...
			<div class="row">
				<div class="col-sm-6">
			      	<h3 class="box-title">Lịch học theo lớp</h3>
                ...
                <div class="col-sm-3">
                    <div class="form-group">
                        <select name="lop" id="lop" class="form-control">
                            ...
                            @foreach($lop as $l2)
                            <option value="{{$l2->MaLop}}">{{$l2->TenLop}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
<script>
   $(document).ready(function(){
    ...
    $(document).on('change','#lop',function(){
        lichlop($('#tuan').val(),$(this).val());
    });

    $(document).on('change','#tuan',function(){
        lichlop($(this).val(),$('#lop').val());
    });

    function lichlop($idTuan, $idLop){
        $.ajax({
            url:'manage/ajax/lichlop?idTuan='+$idTuan+'&MaLop='+$idLop,
            type: 'get',
    ...
    $(document).on('click','#save',function(){
     ...
     $(document).on('click','.sua',function(){
        ...
        var idLich = $(this).data('value');
        $('#capnhat').val(idLich);
        $.ajax({
            url: 'manage/ajax/lichsuaid?idLich='+idLich,
            type: 'get',
            success:function(response){
